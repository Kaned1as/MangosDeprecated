...
{
    if(GetTypeId() == TYPEID_PLAYER && CanUseAttackType(OFF_ATTACK))
...
void Unit::RemoveSpellbyDamageTaken(AuraType auraType, uint32 damage)
{
    ...
    uint32 max_dmg;
    if(auraType == SPELL_AURA_MOD_ROOT && GetMaxHealth() > 4)
        max_dmg = GetMaxHealth()/4;
    else
        max_dmg = getLevel() > 8 ? 25 * getLevel() - 150 : 50;

    //Ranger: taked from getmangos.com (insider42 patch)
    // Glyph of Frost Nova
...
SpellMissInfo Unit::MeleeSpellHitResult(Unit *pVictim, SpellEntry const *spell, bool canMiss)
{
    ...
    if (!pVictim->HasInArc(M_PI_F,this))
    {
        ...
        if (GetTypeId() == TYPEID_PLAYER && pVictim->GetTypeId() == TYPEID_PLAYER)
            canDodge = false;
        // Can`t parry (Deterrence - can parry from behind)
...
SpellMissInfo Unit::MagicSpellHitResult(Unit *pVictim, SpellEntry const *spell)
{
    ...
    // Amaru: Deterrence - no position check
    if (pVictim->HasInArc(M_PI_F,this) || pVictim->HasAura(19263))
    {
        int32 deflect_chance = pVictim->GetTotalAuraModifier(SPELL_AURA_DEFLECT_SPELLS)*100;
        tmp+=deflect_chance;
...
void Unit::RemoveSingleAuraDueToSpellByDispel(uint32 spellId, uint64 casterGUID, Unit *dispeler)
{
    ...
    // Custom dispel cases
    // Lifebloom
    if(spellEntry->SpellFamilyName == SPELLFAMILY_DRUID && (spellEntry->SpellFamilyFlags & UI64LIT(0x1000000000)))
    {
		...
		caster->CastCustomSpell(target, 33778, &heal, NULL, NULL, true, NULL, healAura, healAura->GetCasterGUID());

                int32 returnmana = (spellEntry->ManaCostPercentage * caster->GetCreateMana() / 100) / 2;
                caster->CastCustomSpell(caster, 64372, &returnmana, NULL, NULL, true, NULL, healAura, healAura->GetCasterGUID());
            }
        }
    }
    // Unstable Affliction
    ...
    for (AuraMap::iterator iter = m_Auras.begin(); iter != m_Auras.end(); )
    {
        ...
        if (aur->GetId() == spellId && aur->GetCasterGUID() == casterGUID)
        {
            ...
            if (aur->GetSpellProto()->SpellFamilyName == SPELLFAMILY_SHAMAN && aur->GetSpellProto()->SpellFamilyFlags & UI64LIT(0x40000000000))
            {
                ...
                new_aur->SetStackAmount(aur->GetStackAmount() > 0 ? aur->GetStackAmount() : 1);
		...
		if ((procSpell->Id != 12536) && (procSpell->Id != 12043))
            ...
            switch(dummySpell->Id)
            {
                ...
                case 12848:
                {
                    ...
                    if(target->GetDummyAura(12654))
                        basepoints[0] += target->GetDummyAura(12654)->GetBasePoints();
        ...
        case SPELLFAMILY_GENERIC:
            switch(auraSpellInfo->Id)
            {
                ...
                case 64568:
                {
                    ...
                    triggeredByAura->modStackAmount(-1);
        ...
        if (donePart && bonus->ap_bonus)
        {
            ...
            if (GetTypeId() == TYPEID_PLAYER && spellProto->SpellFamilyName == SPELLFAMILY_DEATHKNIGHT)
            {
                ...
                if (SpellEntry const* spell = ((Player*)this)->GetKnownTalentRankById(2005))
                    ap_bonus += (spell->CalculateSimpleValue(EFFECT_INDEX_0)) / 100.0f;
            }
    ...
    switch(spellProto->DmgClass)
    {
        ...
        case SPELL_DAMAGE_CLASS_RANGED:
        {
            ...
            if (pVictim && spellProto->SpellFamilyName == SPELLFAMILY_DRUID && spellProto->SpellFamilyFlags & UI64LIT(0x0000000000800000) && spellProto->SpellIconID == 1680)
            {
                ...
                for (Unit::AuraList::const_iterator i = mDummyAuras.begin(); i != mDummyAuras.end(); ++i)
                {
                    if ((*i)->GetSpellProto()->SpellFamilyName == SPELLFAMILY_DRUID && (*i)->GetSpellProto()->SpellIconID == 2859)
                    {
                           bonusRaT = (*i)->GetModifier()->m_amount;
                           break;
                    }
                }
                ...
                Unit::AuraList const& mPeriodicDamageAura = pVictim->GetAurasByType(SPELL_AURA_PERIODIC_DAMAGE);
                for(Unit::AuraList::const_iterator i = mPeriodicDamageAura.begin(); i != mPeriodicDamageAura.end(); ++i)
                {
                    if ((*i)->GetSpellProto()->Mechanic == MECHANIC_BLEED || (*i)->GetSpellProto()->EffectMechanic[(*i)->GetEffIndex()] == MECHANIC_BLEED)
                    {
                          bleed_found = true;
                          break;
                    }
...
uint32 Unit::SpellHealingBonusDone(Unit *pVictim, SpellEntry const *spellProto, int32 healamount, DamageEffectType damagetype, uint32 stack)
{
    ...
    //megai2: i guess that (amt + bonus) * donetotalmod = 1 stack damage, so overall = 1 stack damage * stacks ?
    float heal = (healamount + DoneTotal)*DoneTotalMod * int32(stack);
...
bool Unit::IsImmunedToSpellEffect(SpellEntry const* spellInfo, SpellEffectIndex index) const
{
    ...
    if(uint32 aura = spellInfo->EffectApplyAuraName[index])
    {
        ...
        //Immunity mechanic recheck and Bladestorm fix
...
bool Unit::IsTriggeredAtSpellProcEvent(Unit *pVictim, Aura* aura, SpellEntry const* procSpell, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, bool isVictim, bool active, SpellProcEventEntry const*& spellProcEvent )
{
    ...
    if(!isVictim && GetTypeId() == TYPEID_PLAYER)
    {
        if(spellProto->EquippedItemClass == ITEM_CLASS_WEAPON)
        {
            ...
            if(!item || item->IsBroken() || !CanUseAttackType(attType) ||item->GetProto()->Class != ITEM_CLASS_WEAPON || !((1<<item->GetProto()->SubClass) & spellProto->EquippedItemSubClassMask))
                return false;
        }
        else if(spellProto->EquippedItemClass == ITEM_CLASS_ARMOR)
        {
            ...
            if(!item || item->IsBroken() || !CanUseAttackType(OFF_ATTACK) || item->GetProto()->Class != ITEM_CLASS_ARMOR || !((1<<item->GetProto()->SubClass) & spellProto->EquippedItemSubClassMask))
                return false;
        }
    }
