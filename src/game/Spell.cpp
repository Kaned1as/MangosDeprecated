...
SpellCastResult Spell::CheckCast(bool strict)
{
    ...
    if(m_caster->GetTypeId()==TYPEID_PLAYER && ((Player*)m_caster)->HasSpellCooldown(m_spellInfo->Id) && !IsPassiveSpell(m_spellInfo->Id))
    {
    ...
    for (int i = 0; i < 3; ++i)
    {
        switch(m_spellInfo->EffectApplyAuraName[i])
        {
            ...
            {
                // Mind Flay
                if (m_spellInfo->SpellIconID == 548 && m_spellInfo->SpellFamilyName == SPELLFAMILY_PRIEST && m_targets.getUnitTarget())
                {
