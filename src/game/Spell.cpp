...
Spell::Spell( Unit* Caster, SpellEntry const *info, bool triggered, uint64 originalCasterGUID, Spell** triggeringContainer )
{
    ...
    if(m_spellInfo->DmgClass == SPELL_DAMAGE_CLASS_MAGIC && !(m_spellInfo->AttributesEx2 & SPELL_ATTR_EX2_CANT_REFLECTED) 
        && !(m_spellInfo->SchoolMask & SPELL_SCHOOL_MASK_NORMAL)) //don't reflect physical attacks ;)
    {
        for(int j = 0;j < 3; ++j)
        {
    ...
    switch(m_spellInfo->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_MAGE:
        {
            ...
            if (m_spellInfo->SpellFamilyFlags & UI64LIT(0x0000008000000000))
             ...
             if(m_spellInfo->Id == 11129)                   // new Combustion
                AddPrecastSpell(29977);
            break;
        }
        case SPELLFAMILY_PRIEST:
        {
...
SpellCastResult Spell::CheckCast(bool strict)
{
    ...
    if (m_caster->GetTypeId()==TYPEID_PLAYER && !(m_spellInfo->Attributes & SPELL_ATTR_PASSIVE) &&
        ((Player*)m_caster)->HasSpellCooldown(m_spellInfo->Id) || (strict && !m_IsTriggeredSpell && m_caster->GetTypeId() == TYPEID_PLAYER && ((Player*)m_caster)->HasGlobalCooldown(m_spellInfo)))
    {
        if(m_triggeredByAuraSpell)
    ...
    if(Unit *target = m_targets.getUnitTarget())
    {
        ...
        if( m_spellInfo->AttributesEx2 == 0x100000 && (m_spellInfo->AttributesEx & 0x200) == 0x200 && target->HasInArc(M_PI, m_caster) )
        {
			...
			//Exclusion for Throw[2764]:Facing Limitation was removed, but they still use the same, old Ex-Flags
    ...
    for (int i = 0; i < 3; ++i)
    {
        ...
        switch(m_spellInfo->Effect[i])
        {
            ...
            case SPELL_EFFECT_SUMMON:
            {
                ...
                {
                    if(summon_prop->Group == SUMMON_PROP_GROUP_PETS && summon_prop->Type != SUMMON_PROP_TYPE_TOTEM)
                    {
