...
{
    ComandGroup const* cmdGrp = sComandsGroup.LookupEntry<ComandGroup>(sec);
    if (cmdGrp)
      _secFlag = cmdGrp->mask;
    else
      _secFlag = 0;

    //ComandGroup const* cmdGrp = sComandsGroup.LookupEntry<ComandGroup>(i);
    _security = _secFlag & SEC_DEFTYPE;
    if (sock)
    {
            ...
            try
            {
                switch (opHandle.status)
                {
                    ...
                    case STATUS_AUTHED:
                        ...
                        break;
                    case STATUS_NEVER:
                        sLog.outDetail( "SESSION: received not allowed opcode %s (0x%.4X)",
                            LookupOpcodeName(packet->GetOpcode()),
                            packet->GetOpcode());
                        break;
                }
