    ...
    if (isArena() && isRated())
    {
        ...
        if (winner_arena_team && loser_arena_team)
        {
            ...
            for(BattleGroundScoreMap::const_iterator itr = m_PlayerScores.begin(); itr != m_PlayerScores.end(); ++itr)
            {
                ...
                sql_query << "INSERT INTO armory_game_chart VALUES ('" << gameID << "', '" << resultTeamID << "', '" << plr->GetGUID() << "', '" << changeType << "', '" << winner_change << "', '" << resultRating << "', '" << itr->second->DamageDone << "', '" << itr->second->Deaths << "', '" << itr->second->HealingDone << "', '" << itr->second->DamageTaken << "', '" << itr->second->HealingTaken << "', '" << itr->second->KillingBlows << "', '" << m_MapId << "', '" << time(NULL) - m_StartTime/1000 << "', '" << time(NULL) << "')";
...
uint32 BattleGround::GetBonusHonorFromKill(uint32 kills) const
{
    ...
    return (uint32)(MaNGOS::Honor::hk_honor_at_level(GetMaxLevel(), kills) * sWorld.getConfig(CONFIG_FLOAT_RATE_HONOR));
}
