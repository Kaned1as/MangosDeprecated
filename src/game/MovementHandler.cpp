...
bool WorldSession::Anti__ReportCheat(const char* Reason,float Speed,const char* Op,float Val1,uint32 Val2,MovementInfo* MvInfo)
{
    ...
    if (GetPlayer())
    {
        ...
        //second step...
        QueryResult *dbRes = CharacterDatabase.PQuery("SELECT count(acctid) FROM cheaters WHERE acctid = '%u' AND Map NOT IN ('530', '571') AND last_date >= NOW()-5400 ORDER BY entry DESC", Acc);
        if (dbRes)
        {
