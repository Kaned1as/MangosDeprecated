...
bool WorldSession::Anti__ReportCheat(const char* Reason,float Speed,const char* Op,float Val1,uint32 Val2,MovementInfo* MvInfo)
{
    ...
    if (Map == 603 && Reason == "Tele hack" && Speed > 500.0f)
    {
...
void WorldSession::HandleMoveNotActiveMover(WorldPacket &recv_data)
{
    ...
    {
        sLog.outDebug("HandleMoveNotActiveMover: incorrect mover guid: mover is " I64FMT " and should be " I64FMT " instead of " UI64FMTD, _player->m_mover->GetGUID(), _player->GetGUID(), old_mover_guid);
        recv_data.rpos(recv_data.wpos());                   // prevent warnings spam
        return;
    }
