...
m_isRemovedOnShapeLost(true), m_in_use(0), m_deleted(false)
{
    ...
    // Apply periodic time mod
        ...
        if (modOwner->HasAuraType(SPELL_AURA_PERIODIC_HASTE)) {
            Unit::AuraList tmpMap = modOwner->GetAurasByType(SPELL_AURA_PERIODIC_HASTE);
...
Unit *caster, Item* castItem) : Aura(spellproto, eff, currentBasePoints, target, caster, castItem)
{
    if(caster && caster->GetTypeId()==TYPEID_PLAYER && ((Player*)caster)->m_lastSpellTargetGUID && spellproto->EffectApplyAuraName[eff] != 227)
...
void Aura::HandleAuraDummy(bool apply, bool Real)
{
    ...
    else
    {
        ...
        //megai2: remove arcane blast buff on arcane missle end
        ...
        case SPELLFAMILY_WARLOCK:
        {
            ...
            if (GetSpellProto()->SpellIconID == 3172 && (GetSpellProto()->SpellFamilyFlags & UI64LIT(0x0004000000000000)))
            {
                ...
                else
                {
                    ...
                    Unit* caster = GetCaster();
                    if (caster && sSpellStore.LookupEntry(48210)) {
                        m_target->DealHeal(caster, bp0, sSpellStore.LookupEntry(48210));
                        //megai2: and cast visual ^^
                        m_target->CastCustomSpell(caster, 50091, NULL, NULL, NULL, true);
                    }
            ...
            if (GetSpellProto()->SpellFamilyFlags & UI64LIT(0x1000000000))
            {
                if (apply)
                {
                    if (Unit* caster = GetCaster())
                    {
                        ...
                        if (target->GetTypeId() != TYPEID_PLAYER || !((Player*)target)->GetSession()->PlayerLoading())
                        {
                            m_modifier.m_amount = caster->SpellHealingBonusDone(target, GetSpellProto(), m_modifier.m_amount / m_stackAmount, SPELL_DIRECT_DAMAGE) * m_stackAmount;
                            m_modifier.m_amount = target->SpellHealingBonusTaken(caster, GetSpellProto(), m_modifier.m_amount, SPELL_DIRECT_DAMAGE);
                        }
                    }
                }
                ...
                {
                    // Final heal only on dur end, heal on dispell done in Unit.cpp
...
void Aura::HandleAuraModDisarm(bool apply, bool Real)
{
    ...
    AuraType type = (AuraType)m_spellProto->EffectApplyAuraName[m_effIndex];
        ...
        if (Real)
        {
            ...
            if(target->GetAurasByType(SPELL_AURA_MOD_STEALTH).size()<=1)
            {
                ...
                for(Unit::AuraList::const_iterator i = mDummyAuras.begin();i != mDummyAuras.end(); ++i)
                {
                    ...
                    // Overkill
                    else if ((*i)->GetId() == 58426 && (GetSpellProto()->SpellFamilyFlags & UI64LIT(0x0000000000400000) || GetSpellProto()->SpellFamilyFlags & UI64LIT(0x0000000000000800)))
                    {
            ...
            case SPELLFAMILY_WARLOCK:
            {
                if (m_spellProto->TargetAuraState == AURA_STATE_CONFLAGRATE)
                {
                    ...
                    if (aura)
                    {
                        ...
                        m_modifier.m_amount = uint32((damagetick * (4 + Immol)) * 0.14f);//megai2: 0.4/3 = 0.14
                    }
                }
                break;
            }
	...
	if (apply && target->GetTypeId() == TYPEID_PLAYER && target->getClass() == CLASS_DRUID && target->m_form && m_modifier.m_miscvalue == STAT_STAMINA)
	{
		switch (target->m_form)
		{
			...
			case FORM_DIREBEAR:
			{
				...
				if (spellInfo)
					m_modifier.m_amount += spellInfo->CalculateSimpleValue(EFFECT_INDEX_1);
            ...
            switch (GetId())
            {
                ...
                case 48266:                                 // Blood Presence
                {
                    ...
                    if (GetId()==48263 || GetId()==48265)   // Frost Presence or Unholy Presence
                    {
                        ...
                        m_target->CastSpell(m_target, 63611, true, NULL, this);
                    else 
                        m_target->RemoveAurasDueToSpell(63611);
                    ...
                    if (GetId()==48263 || GetId()==48266)   // Frost Presence or Blood Presence
                    {
                        ...
                        if (power_pct)
                            m_target->CastCustomSpell(m_target, 49772, &power_pct, NULL, NULL, true, NULL, this);        // Unholy Presence, speed part, spell1 used for Improvement presence fit to own presence
                        else
...
void Aura::PeriodicTick()
{
    ...
    switch(m_modifier.m_auraname)
    {
	    ...
	    if (savedModAmt != -1)
            ...
            {
                // Only from non-grey units
                //megai2: moved from 10 to 20 since in 10 tryes no shards 
                if (roll_chance_i(20) &&                    // 1-2 from drain with final and without glyph, 0-1 from damage
...
// Exist more after, need add later
            ...
            case SPELLFAMILY_MAGE:         
                ...
                {       //Set name of summons to name of caster
                    ...
                    ((Player*)m_target)->SendMessageToSetInRange(&data, 80.0f, false, false);
...
//                return;
            ...
            if (spell->SpellFamilyFlags & UI64LIT(0x0000000000004000))
            {
                ...
                for(uint32 i = 0; i < MAX_RUNES; ++i)
                {
                    if ((plr->GetCurrentRune(i) == RUNE_FROST || plr->GetCurrentRune(i) == RUNE_UNHOLY) && (plr->GetCurrentRune(i) != deathRuneType))
                    {
                        ...
                        const SpellEntry* usedBy = sSpellStore.LookupEntry( plr->RuneUsedBySpell(i) );                                           
                        if (!usedBy)
                            continue;
            ...
            // Amaru: Hysteria deals damage to caster on tick
