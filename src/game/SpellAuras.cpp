            ...
            if (GetSpellProto()->SpellFamilyFlags & UI64LIT(0x1000000000))
            {
                if (apply)
                {
                    if (Unit* caster = GetCaster())
                        // prevent double apply bonuses
                        if (m_target->GetTypeId() != TYPEID_PLAYER || !((Player*)m_target)->GetSession()->PlayerLoading())
                            m_modifier.m_amount = caster->SpellHealingBonus(m_target, GetSpellProto(), m_modifier.m_amount / m_stackAmount, SPELL_DIRECT_DAMAGE) * m_stackAmount;
                }
                else
                {
    ...
    if (apply)
    {
        ...
        switch (m_spellProto->SpellFamilyName)
        {
            ...
            case  SPELLFAMILY_DEATHKNIGHT:
            {
                // Blood Plague
                // Frost Fever
                if (m_spellProto->Id == 55095 || m_spellProto->Id == 55078)
                {
		...
		if (m_spellProto->TargetAuraState == AURA_STATE_CONFLAGRATE)
		    ...
		    bool Immol = false;
                    ...
                    if (aura)
                    {
                        ...
                        m_modifier.m_amount = uint32((damagetick * (4 + Immol)) * 0.4f);
