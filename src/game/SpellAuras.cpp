...
void Aura::HandleAuraDummy(bool apply, bool Real)
{
    ...
    if(apply)
    {
        switch(GetId())
        {
            ...
            case 59163:
            case 59164:
                if(caster && (m_target->hauntDmg > 0))
                {
                    if (caster->hauntedGuid != m_target->GetGUID())
                    {
                        Unit* oldTgt = caster->GetUnit(*caster, caster->hauntedGuid);
                        if (oldTgt)
                            oldTgt->RemoveAurasDueToSpell(oldTgt->hauntedSId);
                    }
                    caster->hauntedGuid = m_target->GetGUID();
        ...
        switch(GetId())
        {
            ...
            case 59164:
                if(caster)
                {
                    ...
                    //heal
                    const int32 v = m_target->hauntDmg;
                    m_target->CastCustomSpell(caster, 48210, &v, 0,0, true, NULL, NULL, m_target->GetGUID());
                    //visual
                    m_target->CastSpell(caster, 50091, true, NULL, NULL, m_target->GetGUID());
                    m_target->hauntDmg = 0;
                }
                return;
        }
...
void Aura::HandlePeriodicTriggerSpell(bool apply, bool /*Real*/)
{
    ...
    if (!apply)
    {
        ...
        // Arcane Blast
        if (GetCaster() && m_spellProto->SpellFamilyName == SPELLFAMILY_MAGE && (m_spellProto->SpellFamilyFlags & UI64LIT(0x0000000000000800))) // AM
