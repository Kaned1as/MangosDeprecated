...
{
    return float(GetUInt32Value(PLAYER_FIELD_COMBAT_RATING_1 + cr)) / GetRatingCoefficient(cr); //hptfix 2010-change
}
...
Item* Player::GetWeaponForAttack(WeaponAttackType attackType, bool nonbroken, bool useable) const
{
    ...
    if (useable && !CanUseAttackType(attackType))
        return NULL;
...
Item* Player::GetShield(bool useable) const
{
    ...
    if( item->IsBroken() || !CanUseAttackType(OFF_ATTACK))
        return NULL;
